#!/bin/bash
# first dumb script version to index collections
...
\time --append -o "{$LOCAL_INDEXING_DATA}/indexing_${COLLECTION}_timereport.txt"-f "%E" $HADOOP_EXEC jar $HADOOP_JAR_JOB pt.arquivo.indexer.mapreduce.SolrPageDocDriver -D jobName="SolrPageDocs_$COLLECTION" -D mapreduce.job.reduces=$HADOOP_REDUCES_NUM $OUTPUT_LOCATION
evaluate_and_exit

# 4. Copy to local filesystem
# MAKE SURE FOLDER EXIST
echo "Copy to Local Filesystem" >> "${LOCAL_INDEXING_DATA}/indexing_${COLLECTION}_timereport.txt"
$HADOOP_EXEC hdfs -copyToLocal /data/$OUTPUT_LOCATION